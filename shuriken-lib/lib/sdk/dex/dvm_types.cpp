...
DVMFundamental::DVMFundamental(Impl * impl) :
        impl(impl) {
}
...
std::string_view DVMFundamental::get_dalvik_format() const {
    return impl.get()->get_dalvik_format();
}

std::string DVMFundamental::get_dalvik_format_string() const {
    return impl.get()->get_dalvik_format_string();
}
...
types::fundamental_e DVMFundamental::get_fundamental_type() const {
    return impl.get()->get_fundamental_type();
}


DVMClass::DVMClass(Impl* impl) :
...
DVMArray::DVMArray(Impl *impl) : impl(std::unique_ptr<Impl>(impl)) {
}
...
size_t DVMArray::get_array_depth() const {
    return impl.get()->get_array_depth();
}

const DVMType & DVMArray::get_base_type() const {
    return impl.get()->get_base_type();
}
...
const DVMFundamental * shuriken::dex::as_fundamental(const DVMType& type) {
    if (auto ptr = std::get_if<DVMFundamental*>(&type)) {
        return *ptr;
    }
    return nullptr;
}
...
const DVMClass * shuriken::dex::as_class(const DVMType& type) {
    if (auto ptr = std::get_if<DVMClass*>(&type)) {
        return *ptr;
    }
    return nullptr;
}
...
const DVMArray * shuriken::dex::as_array(const DVMType& type) {
    if (auto ptr = std::get_if<DVMArray*>(&type)) {
        return *ptr;
    }
    return nullptr;
}
