...
protected:
    ...
    // We'll need to store our DVMType instances somewhere
...
TEST_F(DVMTypeTest, ArrayType) {
    // Get a reference to our int type
    DVMType &int_type = get_int_type();

    // Create array provider and array
    DVMArrayProvider array_provider{"[I", 1, int_type};
    DVMType array_type{array_provider};  // Explicit variant construction
...
TEST_F(DVMTypeTest, MultiDimensionalArrays) {
    ...
    DVMArrayProvider array1d_provider{"[I", 1, get_int_type()};

    auto array1d_type = std::make_unique<DVMType>(array1d_provider);
    type_storage.push_back(std::move(array1d_type));

    // Create a 2D array using the 1D array as base
    DVMArrayProvider array2d_provider{"[[I", 2, get_int_type()};
...
TEST_F(DVMTypeTest, ArrayOfObjects) {
    ...
    DVMArrayProvider array_provider{"[Ljava/lang/String;", 1, get_string_type()};
